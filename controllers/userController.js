...
exports.posttRegisterUser= async(req,res)=>{
        const body=req.body;
        if(!body.name && !body.email &&!body.password &&!body.gender &&!body.farming){
    ...
    if (err) return res.status(500).json('Error on the server.');
     if (!user) return res.status(404).json({message:'No user found.'});
        ...
        if (!passwordIsValid) return res.status(401).json({ auth: false, token: null , message:`password incorect`});
          ...
          const payload = {id:user.id, name:user.name,email:user.email,location:user.location,farming:user.farming,avater:user.avater}
...
exports.updateUserProfile = async (req, res) => {
  ...
  user.farming =req.body.farming ||user.farming
